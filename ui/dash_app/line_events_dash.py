...
def init_line_events_dashboard(flask_app):
    dash_app = dash.Dash(
        __name__,
        server=flask_app,
        url_base_pathname="/dash/line-events/",
    ...
    dash_app.layout = html.Div(
        [
            html.H2("Line Event Visualization"),
            html.P(
                "Timeline of events per line/machine. "
                "Selectors mirror the OEE dashboard and will drive the timeline later."
            ),
            html.Div(
                [
                    html.Div(
            ...
            dcc.Store(id="le-machines-store"),
            dcc.Interval(
                id="le-machine-loader",
                interval=500,
                n_intervals=0,
                max_intervals=1,
            ),
            ...
            html.Div(
                id="le-content",
                children=html.Div(
                    "Event timeline will be implemented here.",
    ...
    @dash_app.callback(
        Output("le-machines-store", "data"),
        Output("le-department", "options"),
        Output("le-department", "value"),
        Output("le-machine-error", "children"),
        Input("le-machine-loader", "n_intervals"),
    ...
    @dash_app.callback(
        Output("le-line", "options"),
        Output("le-line", "value"),
        Input("le-department", "value"),
        State("le-machines-store", "data"),
        State("le-line", "value"),
    )
    ...
    @dash_app.callback(
        Output("le-machine", "options"),
        Output("le-machine", "value"),
        Input("le-department", "value"),
        Input("le-line", "value"),
        State("le-machines-store", "data"),
        State("le-machine", "value"),
    )
    ...
    @dash_app.callback(
        Output("le-content", "children"),
        Output("le-error", "children"),
        Input("le-refresh", "n_clicks"),
        State("le-machine", "value"),
        State("le-date-range", "start_date"),
        State("le-date-range", "end_date"),
        State("le-start-time", "value"),
        State("le-end-time", "value"),
        prevent_initial_call=False,
    )
    def update_placeholder(
        ...
        return html.Div(
            [
                html.Div(
                    "Line event timeline will be implemented here.",
                    style={
                        "fontWeight": "600",
                        "marginBottom": "8px",
                    },
